...
import com.mygdx.arkanoid.io.KeyListener;
...
public class Arkanoid extends Game {
	...
	public void create() {
		...
		hiloCliente.start();
		teclas = new KeyListener(hiloCliente);
...
//		if(nroJugador==1) {
		if (teclas.isLeft()) {
			...
			hiloCliente.enviarMensaje("Izquier");
...
//		if(nroJugador==1) {
		if (teclas.isRight()) {
			...
			p1.setPosX(posX);
			hiloCliente.enviarMensaje("Derecha");
...
//		}
	...
	public int rebota(Pelota pe, Paleta p, Bloque[] b, int cont) {
		System.out.println("NUMERO PELOTA"+pe.getNroJugador());
